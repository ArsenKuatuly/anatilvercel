...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const lessonsPerModule =
            modulesCount > 0 ? Math.ceil(totalLessons / modulesCount) : 0;

        const completedModules =
            lessonsPerModule > 0
                ? Math.min(
                    modulesCount,
                    Math.floor(completedLessons / lessonsPerModule)
                )
                : 0;

        const modulesLeft = Math.max(modulesCount - completedModules, 0);
