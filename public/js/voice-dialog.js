(function(){
  ...
  function buildFallbackReply(userText, action){
    ...
    const scenario = SCENARIOS[state.scenario] || SCENARIOS.intro;
    if (action === 'hint') {
      return { text:'Мысалы: Мен Алматыдан келдім. Мен студентпін.', translation:'Например: Я приехал из Алматы. Я студент.' };
    }
    if (action === 'explain') {
      return { text:'Қазақ тілінде сөз тәртібі көбіне: бастауыш + толықтауыш + етістік.', translation:'В казахском языке порядок слов часто такой: подлежащее + дополнение + глагол.', explanation:'Старайся ставить глагол ближе к концу предложения.' };
    }
    if (action === 'repeat') {
      return { text:'Қайталап айтамын: өзің туралы қысқа айтып беріңізші.', translation:'Повторяю: коротко расскажите о себе, пожалуйста.' };
    }
    if (scenario.title === 'Знакомство') {
      return { text: lower.includes('аты') ? 'Танысқаныма қуаныштымын. Сіз қай қаладан келдіңіз?' : 'Жақсы. Өзіңіз туралы тағы бір сөйлем айтыңызшы.', translation:'Хорошо. Скажите ещё одно предложение о себе, пожалуйста.' };
    }
    if (scenario.title === 'Кафе') {
      return { text:'Жақсы, тағы не қалайсыз?', translation:'Хорошо, что ещё хотите?' };
    }
    if (scenario.title === 'Магазин') {
      return { text:'Бұл тауар бар. Тағы бір нәрсе керек пе?', translation:'Этот товар есть. Нужно что-то ещё?' };
    }
    if (scenario.title === 'Такси') {
      return { text:'Жақсы, мекенжайды айтыңызшы.', translation:'Хорошо, скажите адрес, пожалуйста.' };
    }
    if (scenario.title === 'Университет') {
      return { text:'Сабақ сағат нешеде басталады деп ойлайсыз?', translation:'Как вы думаете, во сколько начинается занятие?' };
    }
    return { text:'Жақсы, жалғастырайық. Тағы бір сөйлем айтыңыз.', translation:'Хорошо, продолжаем. Скажите ещё одно предложение.' };
  }
